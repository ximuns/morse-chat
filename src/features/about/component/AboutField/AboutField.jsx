...
function AboutField({ progress }) {
  ...
  useEffect(() => {
    ...
    const glowGeometry = new THREE.PlaneGeometry(window.innerWidth * 1.5, 280)
    ...
    for (let i = 0; i < signalCount; i++) {
      ...
      points.push(new THREE.Vector3(startX, y, -30))

      for (let j = 1; j < 9; j++) {
        const x = THREE.MathUtils.lerp(startX, endX, j / 9)

        const yy = y + Math.sin(j * 1.7 + i) * 14

        points.push(new THREE.Vector3(x, yy, -30))
      }

      points.push(new THREE.Vector3(endX, y, -30))
    ...
    const handlePointerMove = (event) => {
      ...
      targetMouse.y = window.innerHeight / 2 - event.clientY
    }
    ...
    const handleResize = () => {
      ...
      glow.geometry = new THREE.PlaneGeometry(window.innerWidth * 1.5, 280)
    }
    ...
    const animate = () => {
      ...
      particles.position.y = -scroll * 850

      particles.position.x = Math.sin(scroll * Math.PI * 2) * 40

      glow.position.y = Math.sin(time * 0.12) * 120 - scroll * 850

      glow.rotation.z = Math.sin(time * 0.08) * 0.04

      signalLines.forEach((line, index) => {
        line.position.y = -scroll * 850 + Math.sin(time * 0.2 + index * 0.4) * 20

        line.rotation.z = Math.sin(time * 0.08 + index) * 0.003
      })
  ...
  return <div ref={containerRef} className="about-field" />
}
