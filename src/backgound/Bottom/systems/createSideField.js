...
function createSideParticle(group, x, y, row, column, side, progress, fadeProgress) {
  ...
  if (column >= 5) {
    ...
    jitterX = (seed2 - 0.5) * (2 + chaos * 9)

    jitterY = (seed3 - 0.5) * (1 + chaos * 5)
  }
  ...
  const size = Math.max(1.6, FIELD.particleSize - column * 0.075)
...
export function createSideField(group, width, height) {
  ...
  const particleStep = FIELD.particleSize + FIELD.particleGap

  const rows = Math.ceil(fieldHeight / particleStep)

  for (let row = 0; row < rows; row++) {
    const y =
      height / 2 - FIELD.firstColumnDistance - FIELD.firstColumnDistance - row * particleStep
    ...
    for (let column = 0; column < columns; column++) {
      const distance = FIELD.firstColumnDistance + column * FIELD.columnGap

      createSideParticle(group, -distance, y, row, column, -1, progress, fadeProgress)
